...
public class FishServer {
	...
	public static void pause(String player) {
		c1.setPause(true);
		for (int b = 0; b < playerThread.size(); b++) {
			writer[b].write(player + " has paused the game!\n");
			writer[b].flush();
		}
		...
		public void run() {
			...
			while (true) {
				if ((playerContinue.size()==playerCount && playerCount>0) || serverContinue) {
					...
					for (int b = 0; b < playerThread.size(); b++) {
						writer[b].write("The game has been continued by all players or the server!\n");
						writer[b].flush();
					}
